  ...
  return (
    ...
    <WrapperHead>
        <div className='wallet'>
            <p className={`${(Balance < totalExpenses) ? 'red-flag' : 'good-track'}`}>Balance:
            <span>{ `${Balance < 0 ? Balance * (-1) : Balance }` }</span></p>
            <p className={`${bankIncome < 0 ? 'red-flag' : 'good-track'}`}>Bank: {bankIncome}</p>
            <p className={`${cashIncome < 0 ? 'red-flag' : 'good-track'}`}>Cash: {cashIncome}</p>
        </div>
        <div>
